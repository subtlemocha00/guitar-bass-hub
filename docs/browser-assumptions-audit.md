...
| **Action taken** | **Built (Phase 5), corrected on-device (two fixes).** Compile-time boolean (no plugin); `RECORD_AUDIO` + `MODIFY_AUDIO_SETTINGS` added to `AndroidManifest.xml`, `NSMicrophoneUsageDescription` to `Info.plist`. Fix 1: a false `"denied"` from the Capacitor `requestPermission` Permissions-API pre-check aborted before `getUserMedia` — pre-check removed so `getUserMedia` is authoritative. Fix 2: `getUserMedia` still rejected `NotAllowedError` on-device because Capacitor's `onPermissionRequest` requests **both** `RECORD_AUDIO` and `MODIFY_AUDIO_SETTINGS` for `AUDIO_CAPTURE` and denies unless every one is granted; `MODIFY_AUDIO_SETTINGS` (a normal permission, auto-granted only if declared) was missing from the manifest, so it added it. Acquire + error-mapping paths verified via CDP; the native OS prompt / bridge grant is the device-only unknown. See [platform-roadmap.md](platform-roadmap.md) and [architecture.md](architecture.md) |
